...
if(isset($_POST['page']))
{
	if($_POST['page'] == 'register')
	{
		...
		if($_POST['action'] == 'register')
		{
			...
			(user_email_address, user_password, user_verfication_code, user_name, user_gender, user_branch, user_roll_no, user_image, user_created_on)
			VALUES 
			(:user_email_address, :user_password, :user_verfication_code, :user_name, :user_gender, :user_branch, :user_roll_no, :user_image, :user_created_on)
			";
			...
			$subject= 'Quizathon Registration Verification';
	...
	if($_POST['page'] == "profile")
	{
		if($_POST['action'] == "profile")
		{
			...
			$exam->data = array(
				...
				':user_gender'				=>	$_POST['user_gender'],
				':user_branch'				=>	$exam->clean_data($_POST['user_branch']),
				':user_email_address'		=>	$exam->clean_data($_POST['user_email_address']),
				':user_roll_no'				=>	$_POST['user_roll_no'],
				':user_image'				=>	$user_image,
			...
			$exam->query = "
			UPDATE user_table 
			SET user_name = :user_name, user_gender = :user_gender, user_branch = :user_branch, user_email_address = :user_email_address, user_roll_no = :user_roll_no, user_image = :user_image 
	...
	if($_POST['page'] == 'index')
	{
		if($_POST['action'] == "fetch_exam")
		{
			...
			foreach($result as $row)
			{
				...
				<tr>
					<td><b>Subject</b></td>
					<td>'.$row["online_exam_subject"].'</td>
				</tr>
				<tr>
					<td><b>Assigned by</b></td>
					<td>'.$row["online_exam_assigned_by"].'</td>
				</tr>
				<tr>
					<td><b>Date & Time</b></td>
	...
	if($_POST["page"] == 'enroll_exam')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			//	$exam->query .= 'online_exam_table.online_exam_subject LIKE "%'.$_POST["search"]["value"].'%" '; 
			...
			foreach($result as $row)
			{
				...
				$sub_array[] = html_entity_decode($row["online_exam_subject"]);
				$sub_array[] = html_entity_decode($row["online_exam_assigned_by"]);
				...
				{
					$status = '<div align="center"><span class="badge badge-warning">Upcoming</span></div>';
				}

				if($row['online_exam_status'] == 'Started')
				{
					$status = '<div align="center"><span class="badge badge-success">Ongoing</span></div>';
				}

				if($row['online_exam_status'] == 'Completed')
				{
					$status = '<div align="center"><span class="badge badge-dark">Completed</span></div>';
				}
				...
				if($row["online_exam_status"] == 'Started')
				{
					$view_exam = '<div align="center"><a href="view_exam.php?code='.$row["online_exam_code"].'" class="btn btn-info btn-sm">View Exam</a></div>';
				}
				if($row["online_exam_status"] == 'Pending')
				{
					$view_exam = '';
				...
				if($row["online_exam_status"] == 'Completed')
				{
					$view_exam = '<div align="center"><a href="view_exam.php?code='.$row["online_exam_code"].'" class="btn btn-primary btn-sm">View Result</a></div>';
				}

				if($row["attendance_status"] == 'Present' && $row["online_exam_status"] == 'Started')
				{
					$view_exam = '<div align="center"><button type="button" class="btn btn-sm btn-warning" disabled>ATTEMPTED !</button></div>';
				}
	...
	if($_POST['page'] == 'view_exam')
	{
		if($_POST['action'] == 'load_question')
		{
			...
			foreach($result as $row)
			{
				...
				<!--change it ayan-->
				<p style="margin: 0; color: #808080;" id="qtn">Question 1</p>
				<p style="margin: 8px; color: rgb(60, 72, 82);">'.$row["question_title"].'</p></div>
				<!--hr /-->
				<br />
				<br />
				...
				foreach($sub_result as $sub_row)
				{
					...
					<div class="col-md-6" style="margin-bottom:22px;">
						<div class="radio">
							<label class="card" style="padding: 24px;"><p style="margin: 0px; color: rgb(60, 72, 82);">
							<input type="radio" name="option_1" class="answer_option" data-question_id="'.$row["question_id"].'" data-id="'.$count.'"/>&nbsp;'.$sub_row["option_title"].'</p></label>
						</div>
					</div>
					';
				...
				$output .= '
					<br /><br />
				  	<div class="row justify-content-around">
				   		<button type="button" name="previous" class="btn btn-warning previous" id="'.$previous_id.'" '.$if_previous_disable.'>Previous</button>
					 	<button type="button" name="next" class="btn btn-success next" id="'.$next_id.'" '.$if_next_disable.'>Save & Next</button>
				  	</div>
					<br /><br />';
			}
		...
		if($_POST['action'] == 'question_navigation')
		{
			...
			foreach($result as $row)	
			{
				...
				<div class="col-md-3.5">
					<button type="button" class="btn btn-success question_navigation" style="background: rgb(255, 255, 255); color: black !important;" data-id="'.$count.'" data-question_id="'.$row["question_id"].'">'.$count.'</button>
				</div>
				';
				$count++;
			}
		...
		if($_POST['action'] == 'user_detail')
		{
			...
			foreach($result as $row)
			{
				$output .= '
				<div align="center">
					<img src="upload/'.$row["user_image"].'" class="img-fluid"  style="margin-top: -20px;"/>
				</div>
