	...
	if($_POST['page'] == 'exam')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			if(isset($_POST['search']['value']))
			{
				...
				$exam->query .= 'OR online_exam_subject LIKE "%'.$_POST["search"]["value"].'%" ';

				$exam->query .= 'OR online_exam_assigned_by LIKE "%'.$_POST["search"]["value"].'%" ';
			...
			foreach($result as $row)
			{
				...
				$sub_array[] = '<div align ="center"><a href="exam_enroll.php?code='.$row['online_exam_code'].'" style="background: #b30086;" class="btn btn-sm">View</a></div>';
		...
		if($_POST['action'] == 'Add')
		{
			$exam->data = array(
				...
				':online_exam_subject'	=>	$exam->clean_data($_POST['online_exam_subject']),
				':online_exam_assigned_by'	=>	$exam->clean_data($_POST['online_exam_assigned_by']),
			...
			(admin_id, online_exam_title, online_exam_subject, online_exam_assigned_by, online_exam_datetime, online_exam_duration, total_question, marks_per_right_answer, marks_per_wrong_answer, online_exam_created_on, online_exam_status, online_exam_code) 
			VALUES (:admin_id, :online_exam_title, :online_exam_subject, :online_exam_assigned_by, :online_exam_datetime, :online_exam_duration, :total_question, :marks_per_right_answer, :marks_per_wrong_answer, :online_exam_created_on, :online_exam_status, :online_exam_code)
			";
		...
		if($_POST['action'] == 'edit_fetch')
		{
			...
			foreach($result as $row)
			{
				...
				$output['online_exam_subject'] = $row['online_exam_subject'];

				$output['online_exam_assigned_by'] = $row['online_exam_assigned_by'];
		...
		if($_POST['action'] == 'Edit')
		{
			$exam->data = array(
				...
				':online_exam_subject'	=>	$_POST['online_exam_subject'],
				':online_exam_assigned_by'	=>	$_POST['online_exam_assigned_by'],
				':online_exam_datetime'	=>	$_POST['online_exam_datetime'] . ':00',
				':online_exam_duration'	=>	$_POST['online_exam_duration'],
				':total_question'		=>	$_POST['total_question'],
				':marks_per_right_answer'=>	$_POST['marks_per_right_answer'],
			...
			SET online_exam_title = :online_exam_title, online_exam_subject = :online_exam_subject, online_exam_assigned_by = :online_exam_assigned_by, online_exam_datetime = :online_exam_datetime, online_exam_duration = :online_exam_duration, total_question = :total_question, marks_per_right_answer = :marks_per_right_answer, marks_per_wrong_answer = :marks_per_wrong_answer  
			...
			$output = array(
				'success'	=>	'Exam Details has been Updated Successfully'
			);
	...
	if($_POST['page'] == 'user')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			if(isset($_POST["search"]["value"]))
			{
			 	...
			 	$exam->query .= 'OR user_roll_no LIKE "%'.$_POST["search"]["value"].'%" ';
			}
			...
			foreach($result as $row)
			{
				...
				$sub_array[] = $row["user_branch"];
				$sub_array[] = $row["user_roll_no"];
				$sub_array[] = $row["user_email_address"];
				$is_email_verified = '';
				if($row["user_email_verified"] == 'yes')
				{
					$is_email_verified = '<label class="badge badge-success">Verified</label>';
				}
				else
				{
					$is_email_verified = '<label class="badge badge-danger">Not Verified</label>';	
				}
	...
	if($_POST['page'] == 'exam_enroll')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			if(isset($_POST['search']['value']))
			{
				...
				$exam->query .= 'OR user_table.user_roll_no LIKE "%'.$_POST["search"]["value"].'%" ';
			...
			foreach($result as $row)
			{
				...
				{
					$result = '<a href="user_exam_result.php?code='.$_POST['code'].'&id='.$row['user_id'].'" class="btn btn-info btn-sm" target="_blank">View</a>';
				}
				$sub_array[] = $result;
