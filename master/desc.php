...
<!--AYAN HERE HEADER OPENS-->

<div class="card"  style="border: none; margin-top: 20px;">
	<div class="card-header">
		<div class="row">
			<div class="col-md-9">
				<h1 class="panel-title" style="margin-top: 0;">ASSIGNMENTS</h1>
			</div>
...
<!--AYAN HERE IS HEADER CLOSED-->

<br />

<!--AYAN HERE ARE GRID AND LIST VIEW BUTTONS -->
...
<!--div class = "col-md-12">
	<h3 class = "grid" style = "float: right; margin-right: 10px;">
		<a href = "desc.php" id="popover" data-toggle="popover" data-trigger="hover" data-placement="bottom" data-content="Gallery View">
			<i class="fas fa-th" id="demo" onclick="myFunction()" style ="margin-right: 10px; color : #000000 !important;"></i>
		</a>
		<a href ="exam.php" id="popover2" data-toggle="popover" data-trigger="hover" data-placement="bottom" data-content="List View">
		<i class="fas fa-th-list" style = "margin-right: 10px; color : grey !important;"></i>
		</a>
	</h3>
</div>
<script>
...
<!--AYAN HERE SEARCH BAR OPENS-->

<div class ="container">
	<div class ="row justify-content-center">
		<input type="text" id="search" placeholder="Search" aria-label="Search" style ="width: 60%;">&nbsp;
		<i class="fas fa-search" type="submit" 
		onclick="search(document.getElementById('search').value)" style ="margin-top: 17px;"></i>
	</div>
</div>
<script>
...
<!--AYAN HERE IS SEARCH BAR CLOSED-->
...
<!--AYAN HERE MAIN CONTENT OPENS-->

<div class = "container">
	<div class = "row mt-4">

	<?php
		$connect = mysqli_connect("localhost","root","","online_examination");	

		$query = "
		SELECT * FROM online_exam_table 
		WHERE admin_id = '".$_SESSION["admin_id"]."'
		ORDER BY online_exam_id desc
		";
		$query_run = mysqli_query($connect, $query);
		$check_exam_table = mysqli_num_rows($query_run) > 0;
		...
		if($check_exam_table){
			while($row = mysqli_fetch_assoc($query_run))
			{
				...
				<!--AYAN HERE ASSIGNMENT CARD OPENS-->
				...
				<!--AYAN HERE ASSSIGNMENT ADDITION DROPDOWN OPENS-->

						<div class="list"  style="float: right;">
							<a class="dropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="background: none;">
							...
							<div class="dropdown-menu">
								...
								if($exam->Is_exam_is_not_started($row["online_exam_id"]))
								{
									...
									echo '
									<div align = "center"><a name="edit" class="edit" id="'.$row['online_exam_id'].'">Edit<br/></a></div>
									';

									echo '<div align = "center"><a name="delete" class="delete" id="'.$row['online_exam_id'].'">Delete</a></div>';
								...
								if($row['online_exam_status'] == 'Completed')
								{
									...
									echo '<div align = "center"><a href="exam_result.php?code='.$row["online_exam_code"].'" class="dropdown-item">View Result</a></div>';
								
									echo '<div align = "center"><a href="exam_enroll.php?code='.$row['online_exam_code'].'" class="dropdown-item">Enrollment List</a></div>';
								...
								{
									echo '<div align = "center"><a name="add_question" class="add_question" id="'.$row['online_exam_id'].'">Add Question</a></div>';
								}
								else
								{
									echo '
									<div align = "center"><a href="question.php?code='.$row['online_exam_code'].'" class="dropdown-item">View Question</a></div>';
								}
							?>
							</div>
							</div>

				<!--AYAN HERE ASSSIGNMENT ADDITION DROPDOWN CLOSED-->

				<!--AYAN HERE ASSIGNMENT DETAILS OPENS-->

							<h4 class = "card-title"><b><?php echo $row['online_exam_title']; ?></b></h4>
							<p class = "card-title"><?php echo $row['online_exam_datetime']; ?></p><br />
							<div class = "container row" style="margin: 0; background: ;">
								<p class = "card-subtitle" style="margin-top: 0; margin-left: 0.8rem;">
									<i class="far fa-clock"></i> <?php echo $row['online_exam_duration']; ?>&nbsp; minutes
								</p>
								<p class = "card-subtitle" style="margin-top: 0; margin-left: 0.8rem;">
									<i class="far fa-file-alt"></i> <?php echo $row['total_question']; ?>&nbsp; questions
								</p>
								<p class = "card-subtitle" style="margin-top: 0; margin-left: 0.8rem;">
									<i class="far fa-check-circle"></i> <?php echo $row['marks_per_right_answer']; ?>&nbsp; marks
								</p>
								<p class = "card-subtitle" style="margin-top: 0; margin-left: 0.8rem;">
									<i class="far fa-times-circle"></i> <?php echo $row['marks_per_wrong_answer']; ?>&nbsp; marks
								</p>
							</div>

				<!--AYAN HERE IS ASSIGNMENT DETAILS CLOSED-->

							<!--AYAN HERE MORE INFO POPOVER OPENS-->

								<a data-toggle="popover" data-placement="bottom" 
								data-content="Subject : <?php echo $row['online_exam_subject']; ?> <br />
								Assigned By : <?php echo $row['online_exam_assigned_by']; ?>" data-html="true">
									More <i class="fas fa-info-circle" style ="margin-right: 10px; color : #000000 !important;"></i>
								</a>
							...
							<!--AYAN HERE IS MORE INFO POPOVER CLOSED-->
								
					<!--AYAN HERE ASSIGNMENT STATUS ACTIVITY OPENS-->
					...
					<!--AYAN HERE IS ASSIGNMENT STATUS ACTIVITY CLOSED-->
				...
				<!--AYAN HERE IS ASSIGNMENT CARD CLOSED-->
				
				<!--AYAN HERE ASSIGNMENT FORM MODAL OPENS-->
				...
				<!--AYAN HERE IS ASSIGNMENT FORM MODAL CLOSED-->


				<!--AYAN HERE ASSIGNMENT DELETE MODAL OPENS-->
				...
				<!--AYAN HERE IS ASSIGNMENT DELETE MODAL CLOSED-->


				<!--AYAN HERE QUESTION FORM MODAL OPENS-->
				...
				<!--AYAN HERE IS QUESTION FORM MODAL CLOSED-->
...
<!--AYAN HERE IS MAIN CONTENT SCRIPT-->

	<script>
			$(document).ready(function(){
				
				var dataTable = $('#desc_data_table').DataTable({
					"processing" : true,
					"serverSide" : true,
					"order" : [],
					"ajax" : {
...
<!--AYAN HERE IS MAIN CONTENT SCRIPT CLOSED-->

		</div>
	</div>
</div>

<!--AYAN HERE IS MAIN CONTENT CLOSED-->

<br /><br /><br />

<!--AYAN HERE FOOTER OPENS-->
